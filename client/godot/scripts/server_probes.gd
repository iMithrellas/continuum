...
func refresh(entries: Array[Dictionary], now := -1.0) -> void:
	...
	for entry in entries:
		...
		_active[key] = {"started": current, "entry": entry}
		state.status = "checking"; _state[key] = state
		probe_started.emit(key)
		if transport.is_valid(): transport.call(entry, Callable(self, "complete" ).bind(key))
...
func complete(key: String, result: Dictionary, now := -1.0) -> void:
