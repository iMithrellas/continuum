...
- [ ] Define a new history-only canonical key from the endpoint scheme/host/port and database identity: lowercase the scheme and DNS host, require brackets around IPv6 literals without address rewriting, remove a port only when it is the explicitly known default for that scheme, preserve unknown or non-default ports, and keep database identity case-sensitive/whitespace-preserving unless provider rules require otherwise; do not reuse or rewrite the existing profile credential key, and do not imply credential migration
